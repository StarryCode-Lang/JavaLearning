...
public class ImageConduct{
    ...
    String msg = this.jTextField.getText();
    ...
    public void Decode(int[][] dc) {
        ...
        for (int i = 0; i < data.length; i += OutputSpeed) {
            for (int j = 0; j < data[0].length; j += OutputSpeed) {
                int[] b = data[i][j];
                if (i < msgByte.length) {
                    b[3] = msgByte[i];
                }
                int result = b[3] << 24 | (b[2] << 16) | (b[1] << 8) | (b[0] << 0);
                //b[3]是空余的
                Color color = new Color(b[2], b[1], b[0]);
                offScreenGraphics.setColor(color);
                offScreenGraphics.fillOval(i / 8, j / 8, 3, 3);
                //bi.getRGB(i,j);
            }
        }
        try{
