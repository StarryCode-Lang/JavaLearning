 ...
 * 3.把字符数据替换成对应的编码
 * 4.把字符编码每8个一组转成byte写入文件中，最后一个字节不足要补全
 * 5.把码表写入文件
 * 6.解压
 */
public class HFMTree {
    ...
    public void number(String datastr) {
        ...
        for (String s : hm.keySet()) {
            TreeNode node = new TreeNode(hm.get(s));
            listNode.add(node);
        }
    }
    ...
    public void inorder(TreeNode root) {
        if (root != null) {
            inorder(root.left);//判断是否是叶子节点
            if (root.left == null && root.right == null) {
                System.out.println(root.data + ":" + root.code);
                codingMap.put(String.valueOf(root.c), root.code);
            }
            inorder(root.right);
        }
    }
